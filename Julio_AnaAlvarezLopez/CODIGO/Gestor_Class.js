...
class Gestor {
    ...
    validateStructure() {
        ...
        if (!Array.isArray(estructura)) {
            return { valid: false, error: 'Estructura no es un array' };
        }
        ...
        const requiredFields = ['name', 'type', 'validations'];
        let validCount = 0;

        for (const attr of estructura) {
            let isValid = true;
        ...
        return {
            valid: validCount === estructura.length,
            totalAttributes: estructura.length,
            validAttributes: validCount,
            attributes: estructura.map(attr => attr.name)
        };
    }
